...
public class RequestDataController {
    ...
    // GET 요청 파라미터 받는 방법 1
    //crossOrigin 을 요청 개별로 걸어줘야함
    //5173 서버로 들어오는 건 다 허용
//    @CrossOrigin("http://localhost:5173")
    ...
    public ResponseEntity<Map<String, String>> reqGet1(HttpServletRequest request) {
        ...
        return ResponseEntity.ok().build();    //.build() - 응답 없음
//        ResponseEntity.status(200)
    }

    // GET 요청 파라미터 받는 방법 2 - 파라미터 받기
    //@RequestParam 이 형변환해줌
    //http://localhost:8080/req/data1?a=김준일&b=32  - 주소에 ?key=value & key=value
    ...
    // GET 요청 파라미터 받는 방법 3 - 변수명과 파라미터명이 일치하면 @RequestParam 의 이름을 생략할 수 있다.
    //@RequestParam 이 형변환해줌
    //http://localhost:8080/req/data1?a=김준일&b=32  - 주소에 ?key=value & key=value
    @GetMapping("/req/data3")
    public ResponseEntity<Map<String, String>> reqGet3(@RequestParam("name") String name, @RequestParam("age") int age) {
    ...
    // 정수 자료형의 파라미터가 필수가 아닐 때 Integer 자료형을 사용해야한다. (null 처리 가능하기 때문에)
    @GetMapping("/req/data4")
    public ResponseEntity<Map<String, String>> reqGet4(@RequestParam(required = false) String name, @RequestParam(required = false) int age) {
    ...
    // GET 요청 파라미터 받는 방법 6 - 파라미터가 많아졌을 때 DTO 로 처리하는 방법
...
//        System.out.println("data6: " + dto.getPhone());
    ...
    // GET 요청 파라미터 받는 방법 6 - 주소에서 값 가져오기
